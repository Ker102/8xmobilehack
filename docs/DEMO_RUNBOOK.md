...
2. **Tap Record (center).** The prompt rotates and the demo story reference image is already attached. "I can speak, and Otto can use the visual clue around the moment." Tap the mic → waveform + timer. Tap again to finish. *(Or tap "type instead" for the most bulletproof path.)*
3. **Generating — the agent at work.** Otto "thinks" while tool-call cards stream in: `transcribe → read_mood → inspect_photos → recall_memory → draft_structure → illustrate → compose_narration`, each going running → done. Call out **`inspect_photos`** identifying handcuffs, siren, moon, and pocket watch from the image, plus **`recall_memory`** for the user's preferred deadpan story style. "A real async agent would emit exactly this trace — transcribe, read the mood, inspect context images, pull from memory, structure, illustrate, narrate. Here it's scripted so it never fails on stage."
4. **Reveal.** The illustrated page plays in with Otto's mood reaction and shows the image Otto used, with an **Add more** affordance for after-the-fact context. "Speak a story → a narrated, illustrated page, in under a minute. This is the magic and the moat."
